...
void loop() {
  ...
  int ledcReadVal = ledcRead(PWM_CHANNEL); // current duty cycle value
  Serial.println((String)"ledcReadVal=" + ledcReadVal);
  double ledcReadFreqVal = ledcReadFreq(PWM_CHANNEL); // current pwm freq
  Serial.println((String)"ledcReadFreqVal=" + ledcReadFreqVal);
  ...
  ledcSetup(PWM_CHANNEL, 600, PWM_RESOLUTION);
}
