 ...
 *  
 *  Requires the Tone32.hpp library in the Makeability Lab Arduino Library
 *  
...
#include <Tone32.hpp>
#include <Wire.h>
// #include <SPI.h> // commented out because we use i2c
...
void loop() {
  ...
  if(_xBall - _ballRadius <= 0 || _xBall + _ballRadius >= _display.width()){
    ...
    // Old way using Arduino: tone(TONE_OUTPUT_PIN, WALL_COLLISION_TONE_FREQUENCY, PLAY_TONE_DURATION_MS);
    _tone32.playTone(WALL_COLLISION_TONE_FREQUENCY, PLAY_TONE_DURATION_MS);
  }
  ...
  if(_yBall - _ballRadius <= 0 || _yBall + _ballRadius >= _display.height()){
    ...
    _tone32.playTone(CEILING_COLLISION_TONE_FREQUENCY, PLAY_TONE_DURATION_MS);
  }
  ...
  _display.display();
  _tone32.update(); // must call this every frame!
