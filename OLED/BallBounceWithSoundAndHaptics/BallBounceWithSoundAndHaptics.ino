...
/**
 *  Draws a bouncing ball on the OLED _display. We play a short tone and 
 *  vibrate a motor when the ball bounces on the "walls" or "ceiling" of the display.
 *  
 *  For the vibromotor, you must use a transistor and the 5V output supply of the Arduino
 *
...
void loop() {
  ...
  if(_xBall - _ballRadius <= 0 || _xBall + _ballRadius >= _display.width()){
    ...
    // Vibrate motor on wall collision
  ...
  if(_yBall - _ballRadius <= 0 || _yBall + _ballRadius >= _display.height()){
    ...
    // Vibrate motor on ceiling coliision
    digitalWrite(VIBROMOTOR_OUTPUT_PIN, HIGH);
    _vibroMotorStartTimeStamp = millis();
  }
