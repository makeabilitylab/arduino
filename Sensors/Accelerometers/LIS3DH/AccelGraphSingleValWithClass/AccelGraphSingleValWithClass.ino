...
void loop() {
  ...
  if(_lastModeButtonVal != graphModeButtonVal && graphModeButtonVal == LOW){
    // Increment which mode we're in
    _graphDataMode = (SensorData)((int)_graphDataMode + 1);

    // Reset back to X_DATA if we go beyond the last mode
    if(_graphDataMode > (int)MAGNITUDE){
  ...
  _lastModeButtonVal = graphModeButtonVal;
